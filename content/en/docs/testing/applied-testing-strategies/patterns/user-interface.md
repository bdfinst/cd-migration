...
| Feature behavior | A flow (login, checkout, search) works through the rendered DOM with the backend stubbed at the network layer | [Component tests]({{< relref "/docs/testing/glossary#component-test" >}}) driven by Playwright with the team's unit-testing framework as the runner |
