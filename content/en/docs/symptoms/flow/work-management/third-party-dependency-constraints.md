...
1. **Is the team's code tightly bound to specific vendor API versions?** If the integration cannot handle multiple vendor versions simultaneously, every vendor change requires a coordinated deployment. Start with [Tightly coupled monolith]({{< relref "/docs/anti-patterns/architecture/tightly-coupled-monolith" >}}).
2. **Must the team coordinate deployment timing with external parties?** If yes, the interfaces between systems do not support independent deployment. Start with [Distributed monolith]({{< relref "/docs/anti-patterns/architecture/distributed-monolith" >}}).
3. **Can the team run old and new integration paths simultaneously?** If switching to a new vendor version is a hard cutover, the pipeline does not support gradual migration. Start with [Missing deployment pipeline]({{< relref "/docs/anti-patterns/pipeline/missing-deployment-pipeline" >}}).
