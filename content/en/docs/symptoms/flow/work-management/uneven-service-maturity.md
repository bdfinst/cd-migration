...
Some services have full [pipelines]({{< relref "/docs/reference/glossary#pipeline" >}}), comprehensive test coverage, automated deployment, and monitoring dashboards. Others have no tests, no pipeline, and are deployed by copying files onto a server. Both sit in the same team's portfolio. The team's [CD]({{< relref "/docs/reference/glossary#cd-continuous-delivery" >}}) practices apply to the modern ones. The legacy ones exist outside them.
...
1. **Does every service in the team's portfolio have an automated deployment pipeline?** If not, identify which services lack pipelines and why. Start with [Missing deployment pipeline]({{< relref "/docs/anti-patterns/pipeline/missing-deployment-pipeline" >}}).
2. **Does the team have time to improve services that are not actively producing incidents?** If improvement work is always displaced by feature or incident work, the team is spread too thin. Start with [Thin-spread teams]({{< relref "/docs/anti-patterns/organizational-cultural/team-dynamics/thin-spread-teams" >}}).
