...
During a [CD]({{< relref "/docs/reference/glossary#cd-continuous-delivery" >}}) migration, teams are building trust in automation. Every flaky test, every
...
the same base images in [CI]({{< relref "/docs/reference/glossary#ci-continuous-integration" >}}) as in production. Pin tool versions explicitly rather than
...
{{< card code=true header="**Immutable build container: Dockerfile with pinned base image and tools**" lang="dockerfile" >}}
...
{{< card code=true header="**Hermetic test environment: GitHub Actions with fresh isolated database per run**" lang="yaml" >}}
...
{{< card code=true header="**Deterministic randomness: fixed seed for predictable test results**" lang="javascript" >}}
...
Determinism is what gives the [single path to production]({{< relref "/docs/migrate-to-cd/pipeline/single-path-to-production" >}})
...
[deployable definition]({{< relref "/docs/migrate-to-cd/pipeline/deployable-definition" >}}) - your quality gates are only as
reliable as the pipeline that enforces them.

When the pipeline is deterministic, [immutable artifacts]({{< relref "/docs/migrate-to-cd/pipeline/immutable-artifacts" >}}) become
...
- [Slow Pipelines]({{< relref "/docs/symptoms/flow/integration/slow-pipelines" >}}) - often worsened by re-runs of non-deterministic failures
- [Snowflake Environments]({{< relref "/docs/anti-patterns/pipeline/snowflake-environments" >}}) - an anti-pattern that introduces environmental variance into the pipeline
- [Immutable Artifacts]({{< relref "/docs/migrate-to-cd/pipeline/immutable-artifacts" >}}) - the Pipeline practice that depends on deterministic builds to be trustworthy
- [Build Duration]({{< relref "/docs/reference/metrics/build-duration" >}}) - a metric directly affected by pipeline determinism and re-run rates
