...
- **Deployment logs:** total deployment count from your [CD]({{< relref "/docs/reference/glossary#cd-continuous-delivery" >}}) platform.
...
- **Pursuing zero defects at the cost of speed.** An obsessive focus on eliminating
  all failures can slow [Release Frequency]({{< relref "/docs/reference/metrics/release-frequency" >}}) to a crawl. A
...
- **Enables confidence in frequent releases.** Teams will only deploy frequently
  ...
  supports higher [Release Frequency]({{< relref "/docs/reference/metrics/release-frequency" >}}).
...
- Implement progressive delivery (canary releases, [feature flags]({{< relref "/docs/reference/glossary#feature-flag" >}})) to limit the
