...
|--------|-----------------|
| [Development cycle time]({{< relref "/docs/reference/metrics/development-cycle-time" >}}) | Reduction in time from story start to verified done, as the QA queue wait disappears |
| [Change fail rate]({{< relref "/docs/reference/metrics/change-fail-rate" >}}) | Should improve as automated tests catch defects before production |
| [Lead time]({{< relref "/docs/reference/metrics/lead-time" >}}) | Decrease as testing no longer adds days or weeks between development and deployment |
| [Integration frequency]({{< relref "/docs/reference/metrics/integration-frequency" >}}) | Increase as developers gain confidence that automated tests catch regressions |
