...
|---------|---------------|--------|
| **Batching** | If deploys are manual, teams batch changes to reduce the number of deploy events | Larger batches increase risk and make [rollback]({{< relref "/docs/reference/glossary#rollback" >}}) harder |
| **Delay** | Changes wait for someone to approve, which may take hours or days | Longer [lead time]({{< relref "/docs/reference/glossary#lead-time-for-changes" >}}), delayed feedback |
...
|-------------|---------------|-------------------|
| **Comprehensive automated tests** | The test suite catches real defects, not just trivial cases | [Testing Fundamentals]({{< relref "/docs/migrate-to-cd/foundations/testing-fundamentals" >}}) |
| **Fast, reliable pipeline** | The pipeline completes in under 15 minutes and rarely fails for non-code reasons | [Deterministic Pipeline]({{< relref "/docs/migrate-to-cd/pipeline/deterministic-pipeline" >}}) |
| **Automated rollback** | You can roll back a bad deployment in minutes without manual intervention | [Rollback]({{< relref "/docs/migrate-to-cd/pipeline/rollback" >}}) |
| **[Feature flags]({{< relref "/docs/reference/glossary#feature-flag" >}})** | Incomplete features are hidden from users via flags, not deployment timing | [Feature Flags]({{< relref "/docs/migrate-to-cd/optimize/feature-flags" >}}) |
| **Small [batch sizes]({{< relref "/docs/reference/glossary#batch-size" >}})** | Each deployment contains 1-3 small changes, not dozens | [Small Batches]({{< relref "/docs/migrate-to-cd/optimize/small-batches" >}}) |
| [**Production-like environments**]({{< relref "/docs/reference/glossary#production-like-environment" >}}) | Test environments match production closely enough that test results are trustworthy | [Production-Like Environments]({{< relref "/docs/migrate-to-cd/pipeline/production-like-environments" >}}) |
| **Observability** | You can detect production issues within minutes through monitoring and alerting | [Metrics-Driven Improvement]({{< relref "/docs/migrate-to-cd/optimize/metrics-driven-improvement" >}}) |
...
4. **Does the team trust the pipeline?** If team members regularly say "let me check one more thing before we deploy," trust is not there yet. Build it through [retrospectives]({{< relref "/docs/migrate-to-cd/optimize/retrospectives" >}}) and transparent metrics.
...
Continuous deployment only increases deploy frequency if the team is integrating to trunk frequently. If the team still merges weekly, they will deploy weekly - automatically, but still weekly. Revisit [Trunk-Based Development]({{< relref "/docs/migrate-to-cd/foundations/trunk-based-development" >}}) and [Small Batches]({{< relref "/docs/migrate-to-cd/optimize/small-batches" >}}).
...
|--------|--------|----------------|
| [Deployment frequency]({{< relref "/docs/reference/metrics/release-frequency" >}}) | Multiple per day | Confirms the pipeline is deploying every change |
| [Lead time]({{< relref "/docs/reference/metrics/lead-time" >}}) | < 1 hour from commit to production | Confirms no manual gates are adding delay |
...
Continuous deployment deploys every change, but not every change needs to go to every user at once. [Progressive Rollout]({{< relref "/docs/migrate-to-cd/continuous-deployment/progressive-rollout" >}}) strategies let you control who sees a change and how quickly it spreads.
...
- [Fear of Deploying]({{< relref "/docs/symptoms/deployment/fear-of-deploying" >}}) - a cultural symptom that fades as automated deployments become routine
- [CAB Gates]({{< relref "/docs/anti-patterns/organizational-cultural/governance-process/cab-gates" >}}) - an organizational anti-pattern that this guide addresses through standard change classification
- [Manual Deployments]({{< relref "/docs/anti-patterns/pipeline/manual-deployments" >}}) - the pipeline anti-pattern that deploy on demand eliminates
- [Deployment Frequency]({{< relref "/docs/reference/metrics/release-frequency" >}}) - the key metric for measuring deploy-on-demand adoption
