...
**What to do:** Define test specifications ([BDD]({{< relref "/docs/reference/glossary#bdd-behavior-driven-development" >}}) scenarios and acceptance criteria) before any code generation. Use a [test fidelity agent]({{< relref "/docs/agentic-cd/operations/pipeline-enforcement" >}}) to validate that generated test code matches the specification. Review agent-generated test code for implementation coupling before approving it.
...
**What to do:** Apply [WIP]({{< relref "/docs/reference/glossary#wip-work-in-progress" >}}) limits to the agent's change queue. If three changes are awaiting review, the agent stops generating new changes until the queue drains. Treat agent-generated review queue depth as a [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}) metric. Consider adopting [expert validation agents]({{< relref "/docs/agentic-cd/operations/pipeline-enforcement" >}}) to handle mechanical review checks, reserving human review for judgment calls.
...
Without provenance tracking, you cannot learn from agent-generated failures, audit agent behavior, or improve the agent's constraints over time. When a production incident involves agent-generated code, you need to know which agent, which [prompt]({{< relref "/docs/reference/glossary#prompt" >}}), and which intent description produced it.

**What to do:** Tag every agent-generated commit with the agent identity, the intent description, and the prompt or [context]({{< relref "/docs/reference/glossary#context-llm" >}}) used. Include provenance metadata in your deployment records. Review agent provenance data during incident retrospectives.
...
{{< card code=true header="**Scope constraint: restrict agent to current scenario only**" >}}
...
{{< card code=true header="**Maintenance session prompt: refactor with no behavior changes**" >}}
...
|--------|--------|----------------|
| Agent-generated [change failure rate]({{< relref "/docs/reference/glossary#change-failure-rate-cfr" >}}) | Equal to or lower than human-generated | Tag agent-generated deployments in your deployment tracker. Compare [rollback]({{< relref "/docs/reference/glossary#rollback" >}}) and incident rates between agent and human changes over rolling 30-day windows. |
