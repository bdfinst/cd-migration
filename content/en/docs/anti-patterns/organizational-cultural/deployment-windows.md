...
|--------|-----------------|
| [Release frequency](../../../metrics/release-frequency/) | Should increase from twice-weekly to daily and eventually on-demand |
| Average changes per deployment | Should decrease as deployment frequency increases |
| [Change fail rate](../../../metrics/change-fail-rate/) | Should decrease as smaller, more frequent deployments replace large batches |
| [Mean time to repair](../../../metrics/mean-time-to-repair/) | Should decrease as deployments happen during business hours with full team awareness |
| [Lead time](../../../metrics/lead-time/) | Should decrease as changes deploy when ready rather than at scheduled windows |
| Emergency change requests | Should decrease as the on-demand deployment process becomes available for all changes |

## Related Content

- [Rollback](../../../migrate-to-cd/migration-path/pipeline/rollback/) - Automated rollback is what makes deployment safe enough to do at any time
- [Single Path to Production](../../../migrate-to-cd/migration-path/pipeline/single-path-to-production/) - One consistent automated path replaces manually staffed deployment events
- [Small Batches](../../../migrate-to-cd/migration-path/optimize/small-batches/) - Smaller deployments are the primary lever for reducing deployment risk
- [Release Trains](release-trains/) - A closely related pattern where a scheduled release window governs all changes
- [Change Advisory Board Gates](cab-gates/) - Another gate-based anti-pattern that creates similar queuing and batching problems
