...
- **They optimize the wrong thing.** Without data, teams default to fixing what is most visible or
  most annoying rather than what is the actual [constraint]({{< relref "/docs/reference/glossary#constraint" >}}).
...
deployment logs, [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}) system, or change management records. If deployments are rare enough that
...
service requiring remediation ([rollback]({{< relref "/docs/reference/glossary#rollback" >}}), hotfix, patch, or incident).
...
3. **Check your version control.** Git log data combined with deployment timestamps gives you
   [lead time]({{< relref "/docs/reference/glossary#lead-time-for-changes" >}}).
...
sophisticated measurement into your pipeline in [Phase 2]({{< relref "/docs/anti-patterns/pipeline" >}}).
{{% /alert %}}
...
|--------|-------------------|---------------|
| Low [deployment frequency]({{< relref "/docs/reference/glossary#deployment-frequency" >}}) + high lead time | Large batches, manual process | [Value Stream Map]({{< relref "/docs/migrate-to-cd/assess/value-stream-mapping" >}}) for queue times |
| High [change failure rate]({{< relref "/docs/reference/glossary#change-failure-rate-cfr" >}}) | Insufficient testing, poor quality practices | [Testing Fundamentals]({{< relref "/docs/migrate-to-cd/foundations/testing-fundamentals" >}}) |
| High [MTTR]({{< relref "/docs/reference/glossary#mean-time-to-restore-mttr" >}}) | No rollback capability, poor observability | [Rollback]({{< relref "/docs/migrate-to-cd/pipeline/rollback" >}}) |
| High lead time + low change failure rate | Excessive manual gates adding delay but not value | [Identify Constraints]({{< relref "/docs/migrate-to-cd/assess/identify-constraints" >}}) |
...
- [Lead Time for Changes]({{< relref "/docs/reference/metrics/lead-time" >}}) - how to measure the time from commit to production
- [Change Failure Rate]({{< relref "/docs/reference/metrics/change-fail-rate" >}}) - measuring the percentage of deployments that cause failures
- [Mean Time to Restore]({{< relref "/docs/reference/metrics/mean-time-to-repair" >}}) - measuring recovery speed after production incidents
- [DORA Recommended Practices]({{< relref "/docs/reference/dora-capabilities" >}}) - the research-backed capabilities these metrics reflect
- [Infrequent Releases]({{< relref "/docs/symptoms/deployment/infrequent-releases" >}}) - a symptom that low deployment frequency baselines often reveal
