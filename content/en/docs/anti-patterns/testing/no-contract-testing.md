...
each team's own [pipeline](../../glossary/#pipeline). The staging environment is the first place where the contract
...
[CD](../../glossary/#cd-continuous-delivery) requires fast, reliable feedback. A shared integration environment that catches contract
...
supports continuous delivery. The first caps [deployment frequency](../../glossary/#deployment-frequency) at integration test cadence.
...
### Step 1: Identify the highest-risk integration points (Week 1)
...
### Step 2: Choose a contract testing approach (Week 1)
...
correctly. The Pact file generated by the test is the contract [artifact](../../glossary/#artifact).
...
### Step 5: Integrate with a contract broker (Week 3-4)
...
### Step 6: Use the "can I deploy?" check before every production deployment (Week 4+)
...
| [Change fail rate](../../../metrics/change-fail-rate/) | Should decrease as production contract violations are caught by pipeline checks |
| [Lead time](../../../metrics/lead-time/) | Should decrease as integration verification no longer requires coordination through a shared environment |
| Service-to-service integrations with contract coverage | Should increase as the practice scales from pilot integrations |
| [Release frequency](../../../metrics/release-frequency/) | Should increase as teams can deploy independently without waiting for integration environment slots |

## Related Content

- [Testing Fundamentals](../../../migrate-to-cd/migration-path/foundations/testing-fundamentals/) - Building the test strategy that includes contract testing
- [Shared Database Across Services](../../../anti-patterns/architecture/shared-database/) - A common cause of implicit contracts that are hard to version
- [Production-Like Environments](../../../migrate-to-cd/migration-path/pipeline/production-like-environments/) - Reducing reliance on shared integration environments
- [Architecture Decoupling](../../../migrate-to-cd/migration-path/optimize/architecture-decoupling/) - Designing service boundaries that make contracts stable
- [Pipeline Architecture](../../../migrate-to-cd/migration-path/pipeline/pipeline-architecture/) - Incorporating contract verification into the deployment pipeline
