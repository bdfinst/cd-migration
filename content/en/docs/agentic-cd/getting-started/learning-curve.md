...
**What it looks like:** Separate specialized agents handle distinct stages of the workflow. A coding agent implements behavior from specifications. Reviewer agents run in parallel to validate test fidelity, architectural conformance, and intent alignment. An [orchestrator]({{< relref "/docs/reference/glossary#orchestrator" >}}) routes work and manages context boundaries. Humans define specifications and review what agents flag - they do not review every generated line.

**What works:** The throughput constraint from Stage 5 is resolved. Expert review agents run at [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}) speed, not human reading speed. Each agent is optimized for its task - the reviewer agents receive only the [artifacts]({{< relref "/docs/reference/glossary#artifact" >}}) relevant to their review, keeping context small and costs bounded. [Token]({{< relref "/docs/reference/glossary#token" >}}) costs are an architectural concern, not a billing surprise.
...
- [Coding & Review Setup]({{< relref "/docs/agentic-cd/architecture/agent-configuration" >}}) - a concrete Stage 6 configuration
- [Small-Batch Sessions]({{< relref "/docs/agentic-cd/architecture/small-batch-sessions" >}}) - how to keep agent context small at every stage
- [Pipeline Enforcement and Expert Agents]({{< relref "/docs/agentic-cd/operations/pipeline-enforcement" >}}) - how review agents replace manual validation at Stage 6
