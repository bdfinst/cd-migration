...
deploy. Any merge during that window could invalidate the tested [artifact]({{< relref "/docs/reference/glossary#artifact" >}}), break the build, or
...
build, test, and deploy atomically eliminate this window because the [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}) handles the full
...
3. **Does the pipeline deploy a specific artifact from a specific commit?** If there is no
   pipeline that pins the deployment to an [immutable artifact]({{< relref "/docs/reference/glossary#immutable-artifact" >}}), the team must manually ensure the
