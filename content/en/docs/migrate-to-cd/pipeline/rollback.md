...
known-good state. It is the safety net that makes [continuous delivery]({{< relref "/docs/reference/glossary#cd-continuous-delivery" >}}) possible: because you
...
rollback verification in your [deployable definition]({{< relref "/docs/migrate-to-cd/pipeline/deployable-definition" >}}) and
...
{{< card code=true header="**Blue-green rollback: traffic switch to previous environment**" lang="text" >}}
Blue (current): v1.2.3
Green (idle):   v1.2.2
...
{{< card code=true header="**Canary rollback: stop routing traffic to the canary on issue detection**" lang="text" >}}
...
{{< card code=true header="**Expand-contract pattern: safe additive schema changes vs. unsafe destructive changes**" lang="sql" >}}
...
- The [single path to production]({{< relref "/docs/migrate-to-cd/pipeline/single-path-to-production" >}}) is how rollback is
  deployed - the same pipeline, the same path, in reverse
- [Immutable artifacts]({{< relref "/docs/migrate-to-cd/pipeline/immutable-artifacts" >}}) are what make rollback reliable - the
  previous artifact is unchanged in the artifact repository, ready to be redeployed
- The [deployable definition]({{< relref "/docs/migrate-to-cd/pipeline/deployable-definition" >}}) should include rollback
  verification as one of its quality gates
- [Application configuration]({{< relref "/docs/migrate-to-cd/pipeline/application-config" >}}) separation ensures that rolling
  back the artifact does not require rolling back environment configuration
- [Production-like environments]({{< relref "/docs/migrate-to-cd/pipeline/production-like-environments" >}}) are where rollback is
...
foundation for [Phase 3: Optimize]({{< relref "/docs/migrate-to-cd/optimize" >}}) and ultimately
...
See [Application Configuration]({{< relref "/docs/migrate-to-cd/pipeline/application-config" >}}) for how to achieve this.
...
- [Manual Deployments]({{< relref "/docs/anti-patterns/pipeline/manual-deployments" >}}) - an anti-pattern incompatible with fast, automated rollback
- [Immutable Artifacts]({{< relref "/docs/migrate-to-cd/pipeline/immutable-artifacts" >}}) - the Pipeline practice that makes rollback reliable by preserving previous artifacts
- [Mean Time to Repair]({{< relref "/docs/reference/metrics/mean-time-to-repair" >}}) - a key metric that rollback capability directly improves
- [Feature Flags]({{< relref "/docs/migrate-to-cd/optimize/feature-flags" >}}) - an Optimize practice that provides an alternative rollback mechanism at the feature level
