...
|--------|-----------------|
| [Lead time]({{< relref "/docs/reference/metrics/lead-time" >}}) | Reduction in time from ready-to-deploy to deployed, as approval wait time decreases |
| [Release frequency]({{< relref "/docs/reference/metrics/release-frequency" >}}) | Increase beyond the once-per-week ceiling imposed by the weekly CAB |
| [Change fail rate]({{< relref "/docs/reference/metrics/change-fail-rate" >}}) | Should stay flat or improve as automated controls catch more issues than manual review |
| [Development cycle time]({{< relref "/docs/reference/metrics/development-cycle-time" >}}) | Decrease as changes no longer batch up waiting for approval windows |
