...
[integration tests]({{< relref "/docs/reference/testing/integration" >}}) still accurately represent the real external system.
...
- You have [integration tests]({{< relref "/docs/reference/testing/integration" >}}) that use test doubles (mocks, stubs, recorded
...
- **Using contract tests to validate business logic**: contract tests verify structure and
  format, not behavior. Business logic belongs in [functional tests]({{< relref "/docs/reference/testing/functional" >}}).
...
Contract tests run **asynchronously** from the main [CI]({{< relref "/docs/reference/glossary#ci-continuous-integration" >}}) build, typically on a schedule:
...
1. **Provider side**: provider contract tests (schema validation, response code checks) are
   ...
   provider's CI [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}).
