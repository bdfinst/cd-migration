...
Hypothesis-driven development treats every change as an experiment. Instead of building features because someone asked for them and hoping they help, teams state a predicted outcome before writing code, measure the result after deployment, and use the evidence to decide what to do next. Combined with [feature flags](../../../glossary/#feature-flag), [small batches](../../../glossary/#batch-size), and [metrics-driven improvement](../metrics-driven-improvement/), this practice closes the loop between shipping and learning.
{{% /pageinfo %}}
...
Build the change using the same [continuous delivery](../../../glossary/#cd-continuous-delivery) practices you use for any other work.

**Use feature flags to control exposure.** The [feature flag](../feature-flags/) infrastructure you built earlier in this phase is what makes experiments possible. Deploy the change behind a flag, then use the flag to control which users see the new behavior.

**Deploy through the standard CD [pipeline](../../../glossary/#pipeline).** Experiments are not special. They go through the same build, test, and deployment process as every other change. This ensures the experiment code meets the same quality bar as production code.

**Keep the change small.** A hypothesis-driven change should follow the same [small batch](../small-batches/) discipline as any other work. If the experiment requires weeks of development, the scope is too large. Break it into smaller experiments that can each be measured independently.
...
| Experiments that are too large | Weeks of development before any measurement | Apply [small batch](../small-batches/) discipline to experiments too |
...
Experiments generate learnings, but learnings only turn into improvements when the team discusses them. [Retrospectives](../retrospectives/) provide the forum where the team reviews experiment results, decides what to do next, and adjusts the process itself.

---

## Related Content

- [Metrics-Driven Improvement](../metrics-driven-improvement/) - the measurement infrastructure that hypothesis-driven development depends on
- [Small Batches](../small-batches/) - the practice that keeps experiments small enough to measure
- [Feature Flags](../feature-flags/) - the mechanism that controls experiment exposure
- [Retrospectives](../retrospectives/) - where the team discusses experiment results and decides next steps
- [First-Class Artifacts](../../../../agentic-cd/specification/first-class-artifacts/) - how ACD formalizes experiment artifacts for agent-assisted workflows
- [Agent-Assisted Specification](../../../../agentic-cd/specification/agent-assisted-specification/) - how agents can help generate and evaluate hypotheses
