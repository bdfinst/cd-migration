...
{{< card code=true header="**Level 1: Static boolean flag in code**" lang="python" >}}
# config.py
...
{{< card code=true header="**Level 2: Dynamic in-process flag service with percentage rollout**" lang="python" >}}
# flag_service.py
import json
...
{{< card code=true header="**Level 2: Flag configuration file with percentage rollout**" lang="json" >}}
{
  "new-checkout": {
    "enabled": true,
    "percentage": 10
...
{{< card code=true header="**Level 3: Centralized flag service with user-context targeting**" lang="python" >}}
...
{{< card code=true header="**Level 4: Istio VirtualService for infrastructure-level traffic routing**" lang="yaml" >}}
...
{{< card code=true header="**Testing both flag states: parametrize over enabled and disabled**" lang="python" >}}
...
{{< card code=true header="**Operational kill switch: disable expensive features during incidents**" lang="python" >}}
...
{{< card code=true header="**Customer entitlement toggle: gate features by subscription level**" lang="python" >}}
...
{{< card code=true header="**Experimentation flag: route users to A/B test variants**" lang="python" >}}
...
Small batches and feature flags let you deploy more frequently, but deploying more means more [work in progress]({{< relref "/docs/reference/glossary#wip-work-in-progress" >}}). [Limiting WIP]({{< relref "/docs/migrate-to-cd/optimize/limiting-wip" >}}) ensures that increased deploy frequency does not create chaos.
...
- [Progressive Rollout]({{< relref "/docs/migrate-to-cd/continuous-deployment/progressive-rollout" >}}) - the deployment strategy that builds on feature flag capabilities
- [Trunk-Based Development]({{< relref "/docs/migrate-to-cd/foundations/trunk-based-development" >}}) - the branching strategy that feature flags enable
- [Limiting WIP]({{< relref "/docs/migrate-to-cd/optimize/limiting-wip" >}}) - the next step after feature flags to manage increased deployment frequency
- [Hypothesis-Driven Development]({{< relref "/docs/migrate-to-cd/optimize/hypothesis-driven-development" >}}) - using feature flags to control experiment exposure
