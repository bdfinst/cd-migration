...
system boundary. External sub-systems are replaced with [test doubles]({{< relref "/docs/reference/testing/test-doubles" >}}) to
...
[end-to-end tests]({{< relref "/docs/reference/testing/e2e" >}}) in that side effects are mocked and never cross boundaries
...
- You need to verify a **complete user-facing feature** from input to output within a single
  [deployable]({{< relref "/docs/reference/glossary#deployable" >}}) unit (e.g., a service or a front-end application).
...
If the test needs to reach a live external dependency, it is an [E2E test]({{< relref "/docs/reference/testing/e2e" >}}). If it
tests a single unit in isolation, it is a [unit test]({{< relref "/docs/reference/testing/unit" >}}).
...
| **Breaks build**| Yes                                                |
| **When to run** | Pre-commit and [CI]({{< relref "/docs/reference/glossary#ci-continuous-integration" >}})                                  |

## Examples

A functional test for a REST API using an in-process server and mocked downstream services:

{{< card code=true header="**REST API functional test - order creation with mocked inventory service**" lang="javascript" >}}
...
{{< card code=true header="**Front-end functional test - login flow with mocked auth service**" lang="javascript" >}}
describe("Login page", () => {
...
{{< card code=true header="**Accessibility functional test - keyboard navigation and axe-core WCAG assertions**" lang="javascript" >}}
...
Functional tests run after unit and integration tests in the [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}), typically as part of
the same CI stage:
...
4. **Pre-deployment gate**: functional tests can serve as the final deterministic gate before
   a build [artifact]({{< relref "/docs/reference/glossary#artifact" >}}) is promoted to a staging environment.
