...
Teams without CAB gates deploy quality checks into the [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}) itself: automated tests, security
...
[Continuous delivery]({{< relref "/docs/reference/glossary#cd-continuous-delivery" >}}) requires that any change can reach production quickly through an automated
pipeline. A weekly approval meeting is fundamentally incompatible with [continuous deployment]({{< relref "/docs/reference/glossary#continuous-deployment" >}}).

The math is simple. If the CAB meets weekly and reviews 20 changes per meeting, the maximum
[deployment frequency]({{< relref "/docs/reference/glossary#deployment-frequency" >}}) is 20 per week. A team practicing CD might deploy 20 times per day. The
...
| Normal | Medium scope, well-tested | New feature behind a [feature flag]({{< relref "/docs/reference/glossary#feature-flag" >}}), API endpoint addition | Peer review + passing pipeline + team lead sign-off |
...
|--------|-----------------|
| [Lead time]({{< relref "/docs/reference/metrics/lead-time" >}}) | Should decrease as CAB delay is removed for standard changes |
| [Release frequency]({{< relref "/docs/reference/metrics/release-frequency" >}}) | Should increase as deployment is no longer gated on weekly meetings |
| [Change fail rate]({{< relref "/docs/reference/metrics/change-fail-rate" >}}) | Should remain stable or decrease - proving auto-approval is safe |
