...
- **Infrastructure platform** - same cloud provider, same [orchestrator]({{< relref "/docs/reference/glossary#orchestrator" >}}), same service mesh
...
production-like environments, the [deployable definition]({{< relref "/docs/migrate-to-cd/pipeline/deployable-definition" >}})
...
[Immutable artifacts]({{< relref "/docs/migrate-to-cd/pipeline/immutable-artifacts" >}}) flow through these environments unchanged,
with only [configuration]({{< relref "/docs/migrate-to-cd/pipeline/application-config" >}}) varying. This combination - same
[artifact]({{< relref "/docs/reference/glossary#artifact" >}}), production-like environment, environment-specific configuration - is what gives
the pipeline its predictive power.

Production-like environments also support effective [rollback]({{< relref "/docs/migrate-to-cd/pipeline/rollback" >}}) testing: you
...
- [Environment-Dependent Failures]({{< relref "/docs/symptoms/testing/environment-dependent-failures" >}}) - test failures rooted in environment parity gaps
- [Snowflake Environments]({{< relref "/docs/anti-patterns/pipeline/snowflake-environments" >}}) - the anti-pattern of manually configured, irreproducible environments
- [Immutable Artifacts]({{< relref "/docs/migrate-to-cd/pipeline/immutable-artifacts" >}}) - the Pipeline practice that flows unchanged through production-like environments
- [Application Configuration]({{< relref "/docs/migrate-to-cd/pipeline/application-config" >}}) - the Pipeline practice that handles the configuration differences between environments
