...
**How to reduce:** Remove manual gates, automate approval workflows, build confidence through progressive rollout. If your [pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}) is reliable (Phase 2), the only thing preventing more frequent deploys is organizational habit.
...
{{< card code=true header="**Day 1 scenario: empty order history endpoint**" lang="gherkin" >}}
...
{{< card code=true header="**Day 2 scenario: return a single order with basic fields**" lang="gherkin" >}}
...
{{< card code=true header="**Day 3 scenario: return orders sorted by date**" lang="gherkin" >}}
...
{{< card code=true header="**Horizontal slicing: stories split by architectural layer**" >}}
...
{{< card code=true header="**Vertical slicing: stories split by user-observable behavior**" >}}
...
The principle is the same. A [subdomain product team]({{< relref "/docs/reference/glossary#subdomain-product-team" >}})'s vertical slice cuts through all layers they control: the service API, the business logic, and the data store. "End-to-end" means end-to-end within your domain, not end-to-end across the entire system. The team deploys independently behind a stable contract, without coordinating with other teams.
...
This usually means the team is still working sequentially. Small stories require limiting [WIP]({{< relref "/docs/reference/glossary#wip-work-in-progress" >}}) and swarming - see [Limiting WIP]({{< relref "/docs/migrate-to-cd/optimize/limiting-wip" >}}). If the team starts 10 small stories instead of 2 large ones, they have not actually reduced batch size; they have increased WIP.

## Measuring Success

| Metric | Target | Why It Matters |
|--------|--------|----------------|
| [Development cycle time]({{< relref "/docs/reference/metrics/development-cycle-time" >}}) | < 2 days per story | Confirms stories are small enough to complete quickly |
| [Integration frequency]({{< relref "/docs/reference/metrics/integration-frequency" >}}) | Multiple times per day | Confirms commits are small and frequent |
...
Small batches often require deploying incomplete features to production. [Feature Flags]({{< relref "/docs/migrate-to-cd/optimize/feature-flags" >}}) provide the mechanism to do this safely.
...
- [Monolithic Work Items]({{< relref "/docs/anti-patterns/team-workflow/monolithic-work-items" >}}) - the anti-pattern of stories too large to deliver in small increments
- [Horizontal Slicing]({{< relref "/docs/anti-patterns/team-workflow/horizontal-slicing" >}}) - the anti-pattern of splitting work by layer instead of by value
- [Work Decomposition]({{< relref "/docs/migrate-to-cd/foundations/work-decomposition" >}}) - the foundational practice for breaking work into small deliverable pieces
- [Feature Flags]({{< relref "/docs/migrate-to-cd/optimize/feature-flags" >}}) - the mechanism that makes deploying incomplete small batches safe
- [Small-Batch Agent Sessions]({{< relref "/docs/agentic-cd/architecture/small-batch-sessions" >}}) - applying the same one-scenario-one-commit discipline to agent-generated work
