...
- **Deployment logs:** timestamps of [rollbacks]({{< relref "/docs/reference/glossary#rollback" >}}) or hotfix deployments.
...
- **Optimizing MTTR without addressing root causes.** Getting faster at fixing
  ...
  better. Pair MTTR with [Change Fail Rate]({{< relref "/docs/reference/metrics/change-fail-rate" >}}) to ensure the
  number of incidents is also decreasing.

## Connection to CD

MTTR is a direct measure of how well the entire [Continuous Delivery]({{< relref "/docs/reference/glossary#cd-continuous-delivery" >}}) system supports
recovery:

- **[Pipeline]({{< relref "/docs/reference/glossary#pipeline" >}}) speed is the floor.** The minimum possible MTTR for a roll-forward
  fix is the [Build Duration]({{< relref "/docs/reference/metrics/build-duration" >}}) plus deployment time. A 30-minute
...
- **Practice improves performance.** Teams that deploy frequently have more
  experience responding to issues. High [Release Frequency]({{< relref "/docs/reference/metrics/release-frequency" >}})
...
- Reduce [Build Duration]({{< relref "/docs/reference/metrics/build-duration" >}}) to enable faster roll-forward.
