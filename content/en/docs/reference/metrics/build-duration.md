...
[Lead Time]({{< relref "/docs/reference/metrics/lead-time" >}}) and directly constrains how quickly a team can
...
Build Duration is a critical bottleneck in the [Continuous Delivery]({{< relref "/docs/reference/glossary#cd-continuous-delivery" >}}) pipeline:
...
- **Constrains Mean Time to Repair.** When production is down, the build pipeline
  ...
  directly improves [MTTR]({{< relref "/docs/reference/metrics/mean-time-to-repair" >}}).
- **Enables frequent integration.** Developers are unlikely to integrate multiple
  ...
  encourage higher [Integration Frequency]({{< relref "/docs/reference/metrics/integration-frequency" >}}).
...
- **Supports [continuous deployment]({{< relref "/docs/reference/glossary#continuous-deployment" >}}).** Automated deployment pipelines cannot deliver
  changes rapidly if the build stage is slow. Build duration is often the largest
  component of [Lead Time]({{< relref "/docs/reference/metrics/lead-time" >}}).
